...
let
  ...
  toSubdomain = server: lib.removeSuffix ".privacy.network" server;

  # File generated by ./pia-server-list-generator.sh
  allServers = import ./pia-generated-server-list.nix;

  filteredServers = builtins.filter (x: lib.elem (toSubdomain x.remote) cfg.serverList) allServers;

  allServerSubdomains = map (x: toSubdomain x.remote) allServers;

  serverEntryToEtcFilename =
    serverEntry:
    let
  ...
  maybeSetupSecrets = lib.optionals (config.system.activationScripts ? setupSecrets) [
    "setupSecrets"
  ];
in
{
  options.networking.networkmanager.pia-vpn = {
    ...
    usernameFile = mkOption {
      ...
      default = null;
      example = "/run/secrets/pia/username";
      description = ''
    ...
    passwordFile = mkOption {
      type = types.nullOr types.path;
      default = null;
      example = "/run/secrets/pia/password";
      description = ''
  ...
  config = mkIf cfg.enable {
    ...
    system.activationScripts.pia-nm-usernameFile = lib.mkIf (cfg.usernameFile != null) (
      stringAfter
        (
          [
            "etc"
    ...
    system.activationScripts.pia-nm-passwordFile = lib.mkIf (cfg.passwordFile != null) (
      stringAfter
        (
          [
            "etc"
